...
# Usage: python simple.py [rngseed], where rngseed is an optional parameter
# specifying the seed of the random number generator.
...
PATTERNSIZE = 1000
# NbNeur = Pattern Size + 1 "bias", fixed-output neuron (bias neuron not
# needed for this task, but included for completeness)
NBNEUR = PATTERNSIZE + 1
...
class NETWORK(nn.Module):
    def __init__(self):
        ...
        self.w = Variable(.01 * torch.randn(NBNEUR, NBNEUR).type(ttype), requires_grad=True)
        # The matrix of plasticity coefficients
        self.alpha = Variable(.01 * torch.randn(NBNEUR, NBNEUR).type(ttype), requires_grad=True)
        # The weight decay term / "learning rate" of plasticity - trainable,
        # but shared across all connections
    ...
    def forward(self, input, yin, hebb):
        ...
        # bmm here is used to implement an outer product between yin and yout, with
        # the help of unsqueeze (i.e. added empty dimensions)
        hebb = (1 - self.eta) * hebb + self.eta * torch.bmm(yin.unsqueeze(2), yout.unsqueeze(1))[0]
        return yout, hebb

    @staticmethod
    def initialZeroState():
...
for numiter in range(2000):
    ...
    if (numiter + 1) % print_every == 0:
        ...
        print(inputs.cpu().numpy()[numstep][0][
              -10:])  # Last input contains the degraded pattern fed to the network at test time
